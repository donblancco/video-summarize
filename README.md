...
**YouTube動画から記事を自動生成するフルスタック・サーバーレスアプリケーション**

![システムUI](img/ui.png)

## 📋 プロジェクト概要

このプロジェクトは、YouTube動画をアップロードするだけで、AI（OpenAI Whisper + GPT-4）を活用して自動的にWordPress記事を生成・投稿するWebアプリケーションです。医療コンテンツ制作の効率化を目的として開発されました。
...
### 🔄 完全自動化ワークフロー
1. **動画アップロード** - シンプルなWebUIから動画を選択
2. **音声抽出** - FFmpegを使用した高品質な音声データ抽出
3. **AI文字起こし** - OpenAI Whisperによる高精度な日本語音声認識
4. **記事生成** - GPT-4による医療専門記事の自動生成
5. **WordPress投稿** - REST APIを使用した完全自動投稿
...
#### 2. 記事生成 (Lambda Function 2)
```python
# GPT-4による記事生成
response = openai.ChatCompletion.create(
    model="gpt-4-turbo",
    messages=[
        {"role": "system", "content": "医療分野のプロライター"},
